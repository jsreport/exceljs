...
class WorkbookWriter {
  ...
  async commit() {
    ...
    const replacedWorksheets = this.templateInfo.sheets.filter(s => s.replaced === true);
    ...
    for (const w of worksheetsToCopy) {
      ...
      await new Promise((resolve, reject) => {
        ...
        parser.onclosetag = function(node) {
          if (
            node.toLowerCase() === 'cols'
          ) {
            placeholderFound = '$colsContent';
          }
        };
        ...
        const distStream = new Transform({
          transform: (chunk, enc, cb) => {
            ...
            if (placeholderFound != null) {
              ...
              toWriteStr = toWriteStr.replace(placeholderFound, colsXML);

              placeholderFound = null;
            }
