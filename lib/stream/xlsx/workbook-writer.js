...
class WorkbookWriter {
  ...
  async commit() {
    ...
    const worksheetReplacementOperations = [];

    // eslint-disable-next-line no-restricted-syntax
    for (const [sheetId, tmpWorksheet] of this._tmpWorksheets.entries()) {
      worksheetReplacementOperations.push(new Promise((resolve, reject) => {
        ...
        parser.onopentag = function(node) {
          if (
            node.name.toLowerCase() === 'col' &&
            node.attributes.WIDTH != null
          ) {
            const colNumber = parseInt(node.attributes.WIDTH.match(/^\$colWidth(\d+)$/)[1], 10);
            const cols = newWorksheetsColsWidth.get(sheetId);
            const col = cols.find((c) => c.number === colNumber);

            if (col.width != null) {
              placeholdersFound.push({
                text: node.attributes.WIDTH,
                value: col.width,
              });
            }
          }
        ...
        const distStream = new Transform({
          transform: (chunk, enc, cb) => {
            ...
            const pastStartPosition = parser.startTagPosition || 0;

            parser.write(chunkStr);

            const startPosition = parser.startTagPosition - (pastStartPosition + lastPendingStr.length);
            let toWriteStr = lastPendingStr + chunkStr.slice(0, startPosition - 1);
            lastPendingStr = chunkStr.slice(startPosition - 1);

            if (placeholdersFound.length > 0) {
              placeholdersFound.forEach((placeholder) => {
                toWriteStr = toWriteStr.replace(placeholder.text, placeholder.value);
              });
          ...
          flush: (cb) => {
            ...
            if (str !== '' || lastPendingStr !== '') {
              cb(null, str + lastPendingStr);
            } else {
              cb();
            }
          },
    ...
    await Promise.all(worksheetReplacementOperations);
