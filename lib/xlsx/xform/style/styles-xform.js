  ...
  parseClose(name) {
    ...
    switch (name) {
      case 'styleSheet': {
        ...
        const add = (_addFn, xform) => {
          if (xform.model && xform.model.length) {
            // eslint-disable-next-line no-restricted-syntax
            for (const model of xform.model) {
              _addFn.call(this, model);
            }
          }
        };

        add(this._addNumFmtStr, this.map.numFmts);
        add(this._addFont, this.map.fonts);
        add(this._addFill, this.map.fills);
        add(this._addBorder, this.map.borders);
        add(this._addStyle, this.map.cellXfs);
